...
def AgentADD_USER(request):
    ...
    if request.method == "POST":
        ...
        if Customer.objects.filter(customer_id = cust_id).exists():
            ...
            customer =  Customer(owner=user,customer_id=cust_id,cust_father_name=father_name,cust_mobileno=mob_no,addresss=addresss,name=username)
...
def  agent_approvedplot(request):
    ...
    booking_data1 = BookPlot.objects.filter(ref_id=request.user.username)
