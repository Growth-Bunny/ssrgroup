...
def ADD_USER(request):
    ...
    if request.method == "POST":
        ...
        if Customer.objects.filter(customer_id = cust_id).exists():
                messages.warning(request, "Adhaar No. is already Taken")
                return redirect('add_user')
        ...
        # customer =  Customer(customer_id=cust_id)
            ...
            customer.save()
            messages.success(request, "Thank You " + user_name +" Your account is successfully created !! Your adhaar no is " + cust_id + " Your Password is " + password)
...
def  addplot(request):
    ...
    if request.method == "POST":
        ...
        plot_size = int(request.POST.get("plot_size"))
        
        # plc_rate = request.POST.get("plc_rate")
        # plc = request.POST.get("plc")
        plot_rate = int(request.POST.get("plot_rate"))
        total_plot_amt = int(plot_size*plot_rate)
        # total_plot_amt = format(total_plot_amt,'.2f')
        # print("totallllllllllll",total_plot_amt)

        total_amt = (plot_rate*plot_size)
        # print("eeeeeeeeeeeeeeeeeeeeee",plc_rate)
        # print("wwwwwwwwwwwwwwwwwwwwwwww",total_plot_amt)
        ...
        add_plot = AddPlot(plot_no= plot_no,plot_size = plot_size, plot_rate = plot_rate,phase=phse_name,t_plot_amt=total_plot_amt,total_amt=total_amt)
...
def VIEWPlotNo(request):
    ...
    plotno = AddPlot.objects.filter(phase="1")
    # plotno = AddPlot.objects.all()
    
    return render(request,'HOD/viewplotno.html',{'plotno':plotno})    

def VIEWPhase2(request):
    
    plotno = AddPlot.objects.filter(phase="2")
    # plotno = AddPlot.objects.all()
    
    return render(request,'HOD/phase2.html',{'plotno':plotno})    

def VIEWPhase3(request):
    
    plotno = AddPlot.objects.filter(phase="3")
    # plotno = AddPlot.objects.all()
    
    return render(request,'HOD/phase3.html',{'plotno':plotno})    
...
def  bookplot(request):
    ...
    if request.method =="POST":
        ...
        if 'demo' in request.POST:
            ...
            booking_amount = (float(request.POST.get('booking_amount')))
            ...
            emi = int(request.POST.get('emi_amt'))

            emi_amt = (amount - booking_amount)/emi
            

           


            center_id = request.POST.get('abc')
            center=AddPlot.objects.get(plot_no=center_id)
            # status = AddPlot.objects.get(status )
            reference_id = CustomUser.objects.get(user_id=ref_id)
            book_plot = BookPlot(ref_id= reference_id,user_id=user_id,plot_number = center, Payable_amout = amount,payment_amount=booking_amount,remaining_amount=remaining_amount, name = name,father_name = father_name , mobile_no = mobile_number, payment_mode = payment_mode ,remarks=remarks,receipt=receipt ,plot_size=plot_size,addresss=addresss,mail=mail,account_no=account_no,ifsc_code=ifsc_code,check_no=check_no,phase=phsee,plc_rate=plc,no_of_emi=emi_amt)
            owner=book_plot.owner=request.user         
            book_plot.owner = owner           
            isntallment = Installment(ref_id= reference_id,user_id=user_id,plot_number = center, Payable_amout = amount,payment_amount = booking_amount,remaining_amount=remaining_amount,name = name, mobile_no = mobile_number, payment_mode = payment_mode ,remarks=remarks,receipt=receipt,plot_size=plot_size ,addresss=addresss,mail=mail,no_of_emi=emi_amt)
...
def UPDATE_CUSTOMER(request):
    if request.method == "POST":
        ...
        # print("usernmaeeeeeeeeee",username)
...
def user_kyc(request):
    kyc = SuperAgent.objects.all()
    return render(request,'HOD/user_kyc.html',{'kyc':kyc})
...
def  ADDInstallment(request):
    ...
    if request.method =="POST":
        if 'newsletter_sub' in request.POST:
            searched = request.POST['searched']
            venues = Installment.objects.filter(plot_number__plot_no__contains=searched)
        ...
        if 'demo' in request.POST:
            ...
            plt = AddPlot.objects.get(plot_no=plot_number)
            # print(ref_id)
            if rem>=0:
                isntallment = Installment(plot_size=plot_size,mail=mail,addresss=addresss,ref_id= ref_id,user_id=user_id,plot_number = plt, Payable_amout = amount,remaining_amount=remaining_amount,payment_amount = payment_amt, name = name, mobile_no = mobile_number, payment_mode = payment_mode ,remarks=remarks,receipt=receipt )
...
def VIEWInstallment(request):
    ...
    # approve = BookPlot.objects.filter(status="1")
    # approve = BookPlot.objects.all()
   
    return render(request,'HOD/view_installment.html',{'isntallment':isntallment})
...
def TOKEN_APPROVE(request,id):
    ...
    approve = AddPlot.objects.get(id=id)
    approve.status = 1
...
def TOKEN_DISAPPROVE(request,id):
    reject = AddPlot.objects.get(id=id)
    reject.status = 2
    reject.save()
    return redirect('token')
