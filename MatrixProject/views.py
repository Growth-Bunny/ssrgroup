...
from matrixapp.models import CustomUser,HOD,SuperAgent,BookPlot,Customer,Installment,AddPlot
...
from matrixapp.pdf import render_to_pdf
...
def dosuperAgent(request):
    ...
    if request.method == "POST":
        ...
        if CustomUser.objects.filter(username = username).exists():
            ...
            agent.save()
            messages.success(request, super_agent.first_name + "  "+ super_agent.last_name + ' Are Successfully Added !!' )
            return render(request, 'preview_user.html',{'user':super_agent,'psw':psw,'agent':agent})
            # return redirect('preview_user',{'user':user})
    context = {
        'code':code,
        'rank':rank
    }
...
def preview_user(request):
    return render(request,"preview_user.html")
...
def doCustomerLogin(request):
    if request.method == "POST":
        ...
        user = EmailBackEnd.authenticate(request, username=request.POST.get('user_id'), password = request.POST.get('password'))
        if user!=None:
            login(request,user)
            user_type = user.user_type
            if user_type == '3':
...
def invoice(request,id):
    ...
    return render(request,'invoice.html',{'plot':plot})

@csrf_exempt
def get_ticket(request):
    ref = request.GET.get("ref")
    ticket1 = AddPlot.objects.get(id=ref)
    data = {
        'ticket1':ticket1
    }
    # return render(request,'invoice.html',{'plot':plot})
